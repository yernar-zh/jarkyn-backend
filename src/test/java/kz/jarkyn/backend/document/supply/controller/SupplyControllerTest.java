...
class SupplyControllerTest {
    ...
    public void testList_success() throws Exception {
        mockMvc.perform(get(Api.Supply.PATH).with(TestUtils.auth())
                ...
                .andExpect(jsonPath("$.sum.currency").isEmpty())
                .andExpect(jsonPath("$.sum.comment").isEmpty())
                .andExpect(jsonPath("$.sum.deleted").isEmpty())
                .andExpect(jsonPath("$.sum.amount").value(710))
                .andExpect(jsonPath("$.sum.moment").isEmpty())
                .andExpect(jsonPath("$.sum.counterparty.name").isEmpty())
                .andExpect(jsonPath("$.sum.counterparty.id").isEmpty())
                .andExpect(jsonPath("$.sum.organization.name").isEmpty())
                .andExpect(jsonPath("$.sum.organization.id").isEmpty())
                .andExpect(jsonPath("$.sum.commited").isEmpty())
                .andExpect(jsonPath("$.sum.warehouse.name").isEmpty())
                .andExpect(jsonPath("$.sum.warehouse.id").isEmpty())
